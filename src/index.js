...
const leftHandListOfNames = document.querySelector('.list-group')
  ...
  function fetchRestaurants() {
    fetch(`http://localhost:3000/api/v1/restaurants?limit=25&page=${page}`)
      .then(r=>r.json())
      .then(r=>{
        console.log(r)
        if (r.error){
...
/////////////////////////////////////////back/next button//////////////////////////////////////////////
  ...
  restaurantDetail.addEventListener('click', (e)=>{
    if (e.target.dataset.action === "update"){
      ...
      allRestaurants.splice(index, 1)
        fetch(`http://localhost:3000/api/v1/restaurants/${clickedId}`,{method: 'DELETE'})
          .then(r=>r.json())
          .then(r=>{
            fetchRestaurants()
          })
  ...
  restaurantDetail.addEventListener('keydown', (e)=>{
    ...
    fetch(`http://localhost:3000/api/v1/restaurants/${clickedId}`,
      { method: 'PATCH',
      headers: {
...
/////////////////////////////////////////////////////////////new review /////////////////////////////////////////////////////////
    document.querySelector('.ui.form').addEventListener('submit', (e)=>{
      ...
      let imageInput = document.querySelector('#image-input').value
        ...
        fetch(`http://localhost:3000/api/v1/restaurants`, {method: 'POST',
          headers: {
              "Content-Type": "application/json",
              "Accept": "application/json"
          },
